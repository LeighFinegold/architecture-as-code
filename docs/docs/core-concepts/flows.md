...
- **`flow-transitions-references-existing-relationship-in-architecture`** — every `relationship-unique-id` in a transition must match an existing relationship in the architecture. A dangling reference is a schema error. This is the enforcement behind the "keeping business and technical views consistent" guarantee: you cannot describe a process step that points at a connection that does not exist.
- **`flow-transitions-have-unique-sequence-numbers`** — sequence numbers within a single flow must be unique. Duplicate numbers make the ordering of steps ambiguous and are rejected as an error.

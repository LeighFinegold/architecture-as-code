...
import { CalmArchitecture } from './architecture';
import {CalmInteracts} from './relationship';
  ...
  "relationships": [
    ...
    },
    {
      ...
      "relationship-type": {
        "connects": {
          ...
          "destination": { "node": "web-gui-process" }
        }
      },
      "protocol": "HTTPS"
    },
    ...
    },
    {
      ...
      "relationship-type": {
        "connects": {
          "source": { "node": "web-gui-process" },
          "destination": { "node": "reference-data-service" }
        }
      },
      "protocol": "HTTPS"
    },
    {
      ...
      "relationship-type": {
        "connects": {
          "source": { "node": "web-gui-process" },
          "destination": { "node": "trading-services" }
        }
      },
      "protocol": "HTTPS"
    },
    ...
    },
    {
      ...
      "relationship-type": {
        "connects": {
          "source": { "node": "web-gui-process" },
          "destination": { "node": "people-service" }
        }
      },
      "protocol": "HTTPS"
    },
    {
      ...
      "relationship-type": {
        "connects": {
          "source": { "node": "people-service" },
          "destination": { "node": "user-directory" }
        }
      },
      "protocol": "LDAP"
    },
...
describe('CalmArchitecture Transformation', () => {
    it('should convert raw JSON into a CalmArchitecture instance with proper nested class instances', () => {

        const plainObject = JSON.parse(rawJson);
        const architectureArray = plainToInstance(CalmArchitecture, [plainObject]);
        const architecture: CalmArchitecture = architectureArray[0];

        expect(architecture).toBeInstanceOf(CalmArchitecture);
        expect(architecture.nodes).toBeDefined();
        expect(Array.isArray(architecture.nodes)).toBe(true);
        expect(architecture.nodes.length).toBe(14);

        const traderxSystem = architecture.nodes.find(
            (node) => node.uniqueId === 'traderx-system'
        );
        expect(traderxSystem).toBeDefined();
        if (traderxSystem) {
            expect(traderxSystem.nodeType).toBe('system');
            expect(traderxSystem.name).toBe('TraderX');
            expect(traderxSystem.description).toBe('Simple Trading System');
        }

        expect(Array.isArray(architecture.flows)).toBe(true);
        expect(architecture.flows.length).toBe(6);

        expect(architecture.relationships.length).toBe(23);

        const interactsRelationships = architecture.relationships.filter((r) =>
            r.relationshipType instanceof CalmInteracts
        );

        expect(interactsRelationships.length).toBe(3);

        const foundRel = interactsRelationships.find(
            (rel) => rel.uniqueId === 'trader-executes-trades'
        );
        expect(foundRel).toBeDefined();

        if (foundRel && foundRel.relationshipType instanceof CalmInteracts) {
            const interactsRel: CalmInteracts = foundRel.relationshipType;
            expect(interactsRel.actor).toBe('traderx-trader');
            expect(interactsRel.nodes).toEqual(['web-client']);
        } else {
            throw new Error('Relationship type is not CalmInteracts');
        }

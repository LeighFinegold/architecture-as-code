...
describe('TemplatePathExtractor', () => {
    ...
    it('gets all top-level nodes', () => {
        const result = TemplatePathExtractor.extract(architectureDocument, 'architecture.nodes');
        expect(result.length).toBe(2);
    });

    it('filters nodes by node-type', () => {
        const result = TemplatePathExtractor.extract(architectureDocument, "architecture.nodes[node-type=='system']");
        expect(result.length).toBe(1);
        expect(result[0]['unique-id']).toBe('account-system');
    });

    it('retrieves nested details.nodes', () => {
        const result = TemplatePathExtractor.extract(architectureDocument, "architecture.nodes['account-system'].details.nodes");
        expect(result.length).toBe(2);
    });

    it('filters nested nodes by node-type', () => {
        const result = TemplatePathExtractor.extract(
            architectureDocument,
            "architecture.nodes['account-system'].details.nodes[node-type=='service']"
        );
        expect(result.length).toBe(1);
        expect(result[0]['name']).toBe('Account Service');
    });
    ...
    it('retrieves interfaces for the UI node', () => {
        ...
        expect(result[0]['url']).toBe('https://account.example.com');
    });
    ...
    it('applies sorting to nested nodes', () => {
        ...
        expect(result[0]['name']).toBe('Account Database');
        expect(result[1]['name']).toBe('Account Service');
    });
    ...
    it('applies extra filter on node name', () => {
        ...
        expect(result.length).toBe(1);
        expect(result[0]['unique-id']).toBe('account-db');
    });
